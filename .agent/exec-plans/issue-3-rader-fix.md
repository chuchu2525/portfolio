...
最終仕様は、desktop が hover preview と click toggle、mobile が tap toggle と outside tap close である。hover スタイルは fine pointer に限定し、mobile では `data-radar-open` による表示だけで開閉する。outside tap close は open 中だけ `window` の `pointerdown` を監視し、mouse pointer は対象外にする。
...
`HeroTurntable.client.tsx` では、button click で `isHeroRadarOpen` を toggle する。`isHeroRadarOpen` が true の間だけ window の `pointerdown` を passive listener として監視し、mouse pointer は無視する。button 外の touch/pen pointer を検知したら close する。
...
動作仕様として、desktop では hover 中にレーダーが表示され、click で `aria-pressed` と `data-radar-open` が切り替わる。mobile では tap で開閉し、開いた状態で Hero 外を tap すると閉じる。Playwright では mobile viewport で `false -> true -> false -> true -> false` の状態遷移と console error なしを確認する。
